...
public class Transaction {
	...
	private String calulateHash() {
		...
		return StringSha256.applySha256(StringSha256.getStringFromKey(sender)
				+ StringSha256.getStringFromKey(reciepient) + Float.toString(value) + sequence);
	}

	// Signs all the data we dont wish to be tampered with.
	public void generateSignature(PrivateKey privateKey) {
		String data = StringSha256.getStringFromKey(sender) + StringSha256.getStringFromKey(reciepient)
				+ Float.toString(value);
		signature = StringSha256.applyECDSASig(privateKey, data);
	}
	...
	public boolean verifiySignature() {
		String data = StringSha256.getStringFromKey(sender) + StringSha256.getStringFromKey(reciepient)
				+ Float.toString(value);
		return StringSha256.verifyECDSASig(sender, data, signature);
	}
